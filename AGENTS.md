...
Fileblend — visual tools for converting and manipulating files. pnpm workspace with `engine` and `website` packages; root scripts orchestrate the workspace.
...
- Run `pnpm type` to check TypeScript types (root `tsc --noEmit` + website type check)
...
- Run `pnpm start:web` to start the engine and website in parallel
